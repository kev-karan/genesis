    ...
    <div style={{ display: 'flex', marginTop: 10, marginLeft: 16, animation: 'fadeDown 0.2s ease' }}>
      <svg width={svgW} height={totalH} style={{ flexShrink: 0, overflow: 'visible' }}>
        <line
          x1={1} y1={filhoY(0)}
          x2={1} y2={filhoY(n - 1)}
          stroke={LINE_COLOR_GRUPOS} strokeWidth={2}
        />
        {drogas.map((_, i) => {
          const cy = filhoY(i)
          return (
...
export default function Protocolo({ protocoloId, navegar }) {
  ...
  const handleToggleFavorite = async () => {
    try {
      if (isFav) {
        await removeFromFavorites(protocoloId)
      } else {
        await addToFavorites(protocoloId)
      }
    } catch (err) {
  ...
  // Recalcula altura da linha vertical (apenas para grupos)
  useEffect(() => {
  ...
  return (
    <div className="screen" style={{ position: 'relative' }}>
      ...
      <div className="content" style={isGruposType ? {} : { position: 'relative', zIndex: 1, flex: 1, overflowY: 'auto' }}>
        ...
        <div style={{
          background: data.conteudo.rootColor || '#1B6FD8',
          color: 'white',
          borderRadius: 12,
          padding: '14px 18px',
          fontSize: 15,
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: GAP_GRUPOS }}>
                {grupos.map((grupo, i) => (
                  <GrupoNode
                    key={grupo.id}
                    grupo={grupo}
                    nodeRef={(ref) => { nodeRefs.current[i] = { current: ref } }}
                  />
                ))}
              </div>
            </div>
